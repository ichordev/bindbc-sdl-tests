...
static uint next_fps_check, frames;
static const uint fps_check_delay = 5000;
...
DrawPoints(SDL_Renderer * renderer)
{
	...
	for (i = 0; i < num_objects * 4; ++i) {
		...
		SDL_SetRenderDrawColor(renderer, 255, cast(ubyte) current_color,
							cast(ubyte) current_color, cast(ubyte) current_alpha);
...
loop()
{
	...
	if (SDL_TICKS_PASSED(now, next_fps_check)) {
		...
		const uint then = next_fps_check - fps_check_delay;
		const double fps = (cast(double) frames * 1000) / (now - then);
