	...
	while (!done) {
		...
		while (SDL_PollEvent(&event)) {
			if (event.type == SDL_DROPBEGIN) {
				SDL_Log("Drop beginning on window %u", cast(uint) event.drop.windowID);
			} else if (event.type == SDL_DROPCOMPLETE) {
				SDL_Log("Drop complete on window %u", cast(uint) event.drop.windowID);
			} else if ((event.type == SDL_DROPFILE) || (event.type == SDL_DROPTEXT)) {
				const (char )*typestr = (event.type == SDL_DROPFILE) ? "File" : "Text";
				char *dropped_filedir = event.drop.file;
				SDL_Log("%s dropped on window %u: %s", typestr, cast(uint) event.drop.windowID, dropped_filedir);
