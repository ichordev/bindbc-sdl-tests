...
*/
//This code was copied from libsdl-org/SDL_mixer version 2.6.0 from GitHub and modified to compile in D.
...
// #endif /* SEEK_TEST */
		...
		case 'h': case 'H':
			...
			break;
		case 'v': case 'V':
			Mix_VolumeMusic(atoi(buf.ptr+1));
			break;
		default:
			break;
		}
	}
	printf("Music playing: %s Paused: %s\n", Mix_PlayingMusic() ? "yes".ptr : "no".ptr,
		Mix_PausedMusic() ? "yes".ptr : "no".ptr);
}
...
int main(int argc, char **argv)
{
	...
	int audio_channels;
	int audio_buffers;
	int audio_volume = MIX_MAX_VOLUME / 32;
	int looping = 0;
	int interactive = 0;
	int rwops = 0;
	int i;
...
// #endif
	...
	if (Mix_OpenAudio(audio_rate, audio_format, audio_channels, audio_buffers) < 0) {
		...
		SDL_Log("Opened audio at %d Hz %d bit%s %s %d bytes audio buffer\n", audio_rate,
			(audio_format&0xFF),
			(SDL_AUDIO_ISFLOAT(audio_format) ? " (float)".ptr : "".ptr),
			(audio_channels > 2) ? "surround".ptr : (audio_channels > 1) ? "stereo".ptr : "mono".ptr,
			audio_buffers);
	}
	audio_open = 1;
