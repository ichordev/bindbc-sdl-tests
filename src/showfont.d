...
*/
//This code was copied from libsdl-org/SDL_ttf version 2.20.0 from GitHub and modified to compile in D.
...
enum DEFAULT_PTSIZE  = 18;
enum WIDTH   = 640;
enum HEIGHT  = 480;

immutable char* TTF_SHOWFONT_USAGE = "Usage: %s [-solid] [-shaded] [-blended] [-utf8|-unicode] [-b] [-i] [-u] [-s] [-outline size] [-hintlight|-hintmono|-hintnone] [-nokerning] [-fgcol r,g,b,a] [-bgcol r,g,b,a] <font>.ttf [ptsize] [text]\n";

alias TextRenderMethod = int;
...
int main(int argc, char **argv)
{
	...
	for (i=1; argv[i] && argv[i][0] == '-'; ++i) {
		...
		if (SDL_strcmp(argv[i], "-fgcol") == 0) {
			int r, g, b, a = 0xFF;
			if (SDL_sscanf(argv[++i], "%d,%d,%d,%d".ptr, &r, &g, &b, &a) < 3) {
				SDL_Log(TTF_SHOWFONT_USAGE, argv0);
				return(1);
			}
			forecol.r = cast(ubyte)r;
			forecol.g = cast(ubyte)g;
			forecol.b = cast(ubyte)b;
			forecol.a = cast(ubyte)a;
		} else
		if (SDL_strcmp(argv[i], "-bgcol") == 0) {
			...
			backcol.r = cast(ubyte)r;
			backcol.g = cast(ubyte)g;
			backcol.b = cast(ubyte)b;
			backcol.a = cast(ubyte)a;
		} else {
			SDL_Log(TTF_SHOWFONT_USAGE, argv0);
			return(1);
		}
	...
	if(dump) {
		for(i = 48; i < 123; i++) {
			...
			glyph = TTF_RenderGlyph_Shaded(font, cast(ushort)i, *forecol, *backcol);

			if(glyph) {
				char[64] outname;
				SDL_snprintf(outname.ptr, (outname).sizeof, "glyph-%d.bmp".ptr, i);
				SDL_SaveBMP(glyph, outname.ptr);
			}
	...
	SDL_snprintf(string_.ptr, (string_).length, "Font file: %s", argv[0]);  /* possible overflow */
	switch (rendermethod) {
	case TextRenderSolid:
		text = TTF_RenderText_Solid(font, string_.ptr, *forecol);
		break;
	case TextRenderShaded:
		text = TTF_RenderText_Shaded(font, string_.ptr, *forecol, *backcol);
		break;
	case TextRenderBlended:
		text = TTF_RenderText_Blended(font, string_.ptr, *forecol);
		break;
	default:
		break;
	}
	...
	if (argc > 2) {
		...
		message = cast(char*)"The quick brown fox jumped over the lazy dog".ptr;
	}
	switch (rendertype) {
		case RENDER_LATIN1:
			switch (rendermethod) {
